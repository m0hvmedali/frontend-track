...
// ملف JavaScript محدث للتصميم الجديد بالبطاقات
...
function displayCommands(tab) {
    ...
    renderCommandsCards(container, commands);  
    console.log(`تم عرض ${commands.length} أمر في تبويب ${tab}`);
}

// رسم بطاقات الأوامر
function renderCommandsCards(container, commands) {
    if (!container) return;
    ...
    commands.forEach((command, index) => {  
        const card = createCommandCard(command, index);  
...
function createCommandCard(command, index) {
    ...
    card.innerHTML = `  
        ...
        <div class="command-type">${escapeHtml(command.type)}</div>  
...
function filterCommands() {
    ...
    renderCommandsCards(container, filtered);  
